  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-xl" id="global-search-bar">
      ...
      <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border transition-all duration-200
        ${open ? 'border-blue-500/60 bg-slate-800 shadow-lg shadow-blue-500/10' : 'border-slate-700/50 bg-slate-800/60 hover:border-slate-600'}`}>
        {loading
        ...
        {query && (
          <button onClick={clear} className="p-0.5 hover:bg-slate-700 rounded-md transition-colors">
      ...
      {/* Dropdown */}
      {open && results && (
        <div className="absolute top-full mt-2 left-0 right-0 z-50 bg-slate-900 border border-slate-700/60 rounded-xl shadow-2xl shadow-black/50 overflow-hidden">
          {/* Meta bar */}
          ...
          <div className="max-h-80 overflow-y-auto">
            ...
            {results.airports.length > 0 && (
              <div>
                <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-800/30">
                ...
                {results.airports.map(a => (
                  <div key={a.iata_code || a.icao_code} className="flex items-center gap-3 px-3 py-2.5 hover:bg-slate-800/60 cursor-default transition-colors border-b border-slate-800/50">
            ...
            {results.aircraft.length > 0 && (
              <div>
                ...
                {results.aircraft.map(a => (
                  <div key={a.icao24} className="flex items-center gap-3 px-3 py-2.5 hover:bg-slate-800/60 cursor-default transition-colors border-b border-slate-800/50">
                    <div className="w-8 h-8 rounded-lg bg-emerald-500/15 flex items-center justify-center shrink-0">
                      <span className="text-emerald-400 font-bold text-[10px]">{a.typecode || '?'}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-white text-xs font-medium truncate">{a.registration} · {a.manufacturer} {a.model}</p>
            ...
            {results.flights.length > 0 && (
              <div>
                ...
                {results.flights.map(f => (
                  <div key={f.flight_iata} className="flex items-center gap-3 px-3 py-2.5 hover:bg-slate-800/60 cursor-default transition-colors border-b border-slate-800/50">
                    <div className="w-8 h-8 rounded-lg bg-purple-500/15 flex items-center justify-center shrink-0">
                      <Plane size={12} className="text-purple-400" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-white text-xs font-medium font-mono">{f.flight_iata}</p>
